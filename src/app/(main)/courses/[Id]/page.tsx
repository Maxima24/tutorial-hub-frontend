...
import { useGetvideoById } from '@/service/mutations/video';

export default function TutorialDetailPage() {
  const { Id } = useParams();
  const tutorialId = Array.isArray(Id) ? Id[0] : Id;
  const router = useRouter();
  const [tutorialData,setTutorialData] = useState<Record<any,any> | null>(null)
  // const { searchedTutorial: tutorial, getTutorialById } = useTutorialsStore();
  const {mutateAsync:getTutorialData,isPending} = useGetvideoById()
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 flex flex-col gap-6">
                  ...
                  <h1 className="text-4xl md:text-5xl font-bold">{tutorialData?.title}</h1>
                  <p className="text-xl text-blue-100">{tutorialData?.description}</p>

                  <div className="flex flex-wrap items-center gap-6">
                    <div className="flex items-center gap-2">
                      <Star className="fill-yellow-400 text-yellow-400" size={20} />
                      <span className="font-bold text-lg">{tutorialData?.rating}</span>
                      <span className="text-blue-100">{tutorialData?.reviews.length}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Users size={20} />
                  ...
                  <div className="flex items-center gap-4 mt-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 bg-gradient-to-br from-pink-400 to-rose-400 rounded-full flex items-center justify-center text-white font-bold">
                        {tutorialData?.instructor.avatar}
                      </div>
                      <div>
                        <div className="font-semibold">Created by {tutorialData?.instructor.name}</div>
                ...
                <div className="lg:col-span-1">
                  <div className="bg-white rounded-2xl shadow-2xl overflow-hidden sticky top-8">
                    <div className={`${tutorialData?.thumbnail} h-48 flex items-center justify-center relative group cursor-pointer`}>
                    ...
                    <div className="p-6">
                      <div className="flex items-baseline gap-3 mb-6">
                        <div className="text-4xl font-bold text-gray-900">${tutorialData?.price}</div>
                        <div className="text-xl text-gray-400 line-through">${tutorialData?.originalPrice}</div>
                  ...
                  <div className="space-y-8">
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {tutorialData?.WhatYouLearn.map((item, idx) => (
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {tutorialData?.requirements.map((req, idx) => (
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="text-sm text-gray-600">
                        {tutorialData?.curriculum.length} modules • {tutorialData?.curriculum.reduce((acc, m) => acc + m.lessons.length, 0)} lessons
                      </div>
                    </div>

                    {tutorialData?.curriculum.map((module, moduleIdx) => (
                ...
                {activeTab === 'instructor' && (
                  <div>
                    ...
                    <div className="flex items-start gap-6 mb-8">
                      ...
                      <div>
                        <h3 className="text-2xl font-bold text-gray-900 mb-2">{tutorialData?.instructor.name}</h3>
                        <p className="text-gray-600 mb-4">{tutorialData?.instructor.title}</p>
                        <div className="flex flex-wrap gap-6 text-sm">
                          <div className="flex items-center gap-2">
                            <Star className="fill-yellow-400 text-yellow-400" size={16} />
                            <span className="font-semibold">{tutorialData?.instructor.rating} Instructor Rating</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Users size={16} />
                            <span>{tutorialData?.instructor.students} Students</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Play size={16} />
                ...
                {activeTab === 'reviews' && (
                  <div>
                    ...
                    <div className="space-y-6">
                      {tutorialData?.reviews.map((review, idx) => (
